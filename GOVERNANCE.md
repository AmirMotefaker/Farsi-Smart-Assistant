...
This document defines the repository-level governance contract for `AmirMotefaker/Farsi-Smart-Assistant`.
...
This repository produces deterministic browser release artifacts.

Product tags matching `v*` use `.github/workflows/release-provenance.yml` to:

1. build and verify the deterministic release artifacts,
2. audit store readiness,
3. generate signed GitHub artifact attestations for the ZIP files, checksums, and release manifest,
4. upload the attested release bundle as a workflow artifact.

Attestations can be verified with GitHub CLI.
