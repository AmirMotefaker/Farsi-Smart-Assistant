document.addEventListener('DOMContentLoaded', function() {
    ...
    const sanitizeHTML = (html) => { const doc = new DOMParser().parseFromString(html, 'text/html'); doc.querySelectorAll('script,style,object,embed,iframe').forEach(el => el.remove()); return doc.body.innerHTML; };
    const renderResult = (result, term) => { knowledgePanel.innerHTML = ''; if (!result) { const p = document.createElement('p'); p.textContent = `نتیجه‌ای برای «${term}» یافت نشد.`; knowledgePanel.appendChild(p); return; } const title = document.createElement('h4'); title.textContent = result.title; knowledgePanel.appendChild(title); const summary = document.createElement('div'); summary.id = 'knowledgeSummary'; summary.innerHTML = sanitizeHTML(result.summary); knowledgePanel.appendChild(summary); };
