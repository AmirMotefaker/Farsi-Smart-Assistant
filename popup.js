document.addEventListener('DOMContentLoaded', function() {
    ...
    const versionDisplay = document.getElementById('version-display'); // عنصر جدید
    ...
    async function init() {
        // خواندن و نمایش ورژن از مانیفست
    ...
    // ==========================================================
    // فانکشن اصلی که تمام کارها را انجام می‌دهد (بدون تغییر)
    // ==========================================================
    ...
    // --- تمام توابع کمکی دیگر (بدون تغییر) ---
    ...
    const renderResult = (result, term) => { knowledgePanel.innerHTML = ''; if (!result) { const title = document.createElement('div'); title.textContent = `نتیجه‌ای برای «${term}» یافت نشد.`; knowledgePanel.appendChild(title); return; } const title = document.createElement('h4'); title.textContent = result.title; knowledgePanel.appendChild(title); if (result.type === 'disambiguation') { const summary = document.createElement('div'); summary.innerHTML = result.summary; knowledgePanel.appendChild(summary); } else { const summary = document.createElement('div'); summary.id = 'knowledgeSummary'; summary.innerHTML = result.summary; knowledgePanel.appendChild(summary); } };
