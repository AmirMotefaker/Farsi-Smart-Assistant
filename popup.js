...
/* FSA v4.8.0 current-site favicon polish */
(function installFsaCurrentSiteFaviconPolish() {
  if (window.__fsaCurrentSiteFaviconPolishInstalled) {
    return;
  }

  window.__fsaCurrentSiteFaviconPolishInstalled = true;

  const imageSelectors = [
    '.current-site-favicon img',
    '.current-site__favicon img',
    '.site-favicon img',
    '.site-favicon-wrap img',
    '.site-logo img',
    '.page-favicon img',
    '.current-page-icon img',
    '.current-site .favicon img',
    '[data-current-site-favicon] img',
    '[data-favicon-container] img',
    'img[src*="favicon"]',
  ...
  const wrapperSelectors = [
    ...
    '.current-page-icon',
    '.favicon',
    '[data-current-site-favicon]',
    '[data-favicon-container]'
  ].join(',');
  ...
  function normalizeWrapper(wrapper) {
    ...
    for (const child of [...wrapper.children]) {
      ...
      const dataAttrs = (
        (child.getAttribute('data-badge') || '') +
        ' ' +
        (child.getAttribute('data-indicator') || '')
      ).toLowerCase();
    ...
    for (const extra of wrapper.querySelectorAll(
      '.badge, .overlay, .indicator, .status, .status-badge, .dot, .ring, [data-badge], [data-indicator], [aria-hidden="true"]'
    )) {
      if (extra.tagName !== 'IMG') {
        extra.remove();
      }
    }
  }
  ...
  function normalizeImage(img) {
    ...
    const wrapper =
      img.closest(wrapperSelectors) ||
      img.parentElement;

    normalizeWrapper(wrapper);
  }

  function applyPolish(root = document) {
    const seen = new Set();
  ...
  const scheduleApply = () => {
    window.requestAnimationFrame(() => applyPolish(document));
  };
